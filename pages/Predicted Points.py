...
if json_files:
    ...
    git_parts = latest_file.replace(fixtures_dir, '').split('_')
    git_timestamp = f"{int(git_parts[1][2:])}.{int(git_parts[1][:2])} {int(git_parts[2][:2])}:{int(git_parts[2][2:])}"
    ...
    if upload_new_file_button:
        ...
        if uploaded_file:
            parts = uploaded_file.replace(fixtures_dir, '').split('_')
            timestamp = f"{int(parts[1][2:])}.{int(parts[1][:2])} {int(parts[2][:2])}:{int(parts[2][2:])}"
