...
if json_files:
    ...
    git_timestamp = f"{int(git_parts[0][3:])}.{int(git_parts[0][:3])} {int(git_parts[1][:3])}:{int(git_parts[1][3:])}"
    ...
    if upload_new_file_button:
        ...
        if uploaded_file:
            ...
            timestamp = f"{int(git_parts[0][3:])}.{int(parts[0][:3])} {int(parts[1][:3])}:{int(parts[1][3:])}"
    ...
    if uploaded_file:
        parts = uploaded_file.replace(filename, '').replace(".json", '').split('_')
        timestamp = f"{int([0][3:])}.{int(parts[0][:3])} {int(parts[1][:3])}:{int(parts[1][3:])}"
        latest_file = uploaded_file
